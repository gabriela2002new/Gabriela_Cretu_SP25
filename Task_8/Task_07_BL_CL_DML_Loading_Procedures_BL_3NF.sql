...
WHERE
    ...
    AND
    (
        CE_CITIES.state_id IS DISTINCT FROM EXCLUDED.state_id
        OR
        CE_CITIES.city_name IS DISTINCT FROM EXCLUDED.city_name
    )
;
    ...
    LOOP
        BEGIN
            ...
            WHERE
                ...
                AND (
                    CE_LOCATION_NAMES.zip_id IS DISTINCT FROM EXCLUDED.zip_id
                    OR CE_LOCATION_NAMES.location_name IS DISTINCT FROM EXCLUDED.location_name
                );
...
select * from bl_cl.lkp_customers lc where customer_src_id='EDNA_036';
SELECT * FROM bl_cl.f_get_merged_customers_scd() ct  where customer_src_id='EDNA_036';
select * from bl_3nf.ce_zip cz where cz.zip_name='50829';
SELECT zip_id
            FROM bl_3nf.CE_ZIP
            WHERE zip_name = 'n. a.'
            LIMIT 1;
...
drop procedure if exists bl_cl.p_load_ce_customers_scd();
    ...
    LOOP
        BEGIN
            ...
            IF EXISTS (
                ...
                -- Check if all attributes are the same (no changes)
                IF EXISTS (
                    SELECT 1
                    FROM bl_3nf.CE_CUSTOMERS_SCD
                    WHERE customer_src_id = rec.customer_src_id
