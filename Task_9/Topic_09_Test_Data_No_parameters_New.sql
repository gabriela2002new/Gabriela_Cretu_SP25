...
CALL bl_dm.load_fct_sales_rolling_window_partitioned();---with partitions inside eac loop
call bl_dm.load_fct_sales_rolling_window_no_partition();

select count(*) from bl_dm.fct_sales_partitioned fsp  ;
select count(*) from bl_dm.fct_sales_no_partition fsnp  ;
...
WHERE procedure_name IN (
    ...
    'sp_insert_fct_sales_incremental_partitioned',
    'sp_insert_fct_sales_incremental_no_partition'
)
...
select * from bl_dm.fct_sales_partitioned fsp  ;
select * from bl_dm.fct_sales_no_partition fsnp  ;
...
SELECT
    ...
    event_dt,
    COUNT(*) AS row_count
FROM bl_dm.fct_sales_partitioned
GROUP BY
    game_number_surr_id,
    customer_surr_id,
    employee_surr_id,
...
SELECT 
    ...
    fs.event_dt
FROM bl_dm.fct_sales_partitioned fs
...
SELECT
    ...
    event_dt,
    COUNT(*) AS row_count
FROM bl_dm.fct_sales_no_partition
GROUP BY
    game_number_surr_id,
    customer_surr_id,
    employee_surr_id,
...
SELECT 
    ...
    dp.payment_method_src_id,
    fs.event_dt
FROM bl_dm.fct_sales_no_partition fs
LEFT JOIN bl_dm.dim_customers_scd dc ON fs.customer_surr_id = dc.customer_surr_id
LEFT JOIN bl_dm.dim_game_numbers dgn ON fs.game_number_surr_id = dgn.game_number_surr_id
LEFT JOIN bl_dm.dim_employees de  ON fs.employee_surr_id = de.employee_surr_id
LEFT JOIN bl_dm.dim_retailer_license_numbers dr ON fs.retailer_license_number_surr_id = dr.retailer_license_number_surr_id
