...
CREATE table if not exists bl_dm.fct_sales_partitioned (
    ...
    CONSTRAINT fct_sales_partitioned_grain_unique UNIQUE (
        game_number_surr_id,
        customer_surr_id,
        employee_surr_id,
...
CREATE table if not exists bl_dm.fct_sales_no_partition (
    game_number_surr_id BIGINT REFERENCES bl_dm.dim_game_numbers(game_number_surr_id) ON DELETE CASCADE,
    customer_surr_id BIGINT REFERENCES bl_dm.dim_customers_scd(customer_surr_id) ON DELETE CASCADE,
    employee_surr_id BIGINT REFERENCES bl_dm.dim_employees(employee_surr_id) ON DELETE CASCADE,
    retailer_license_number_surr_id BIGINT REFERENCES bl_dm.dim_retailer_license_numbers(retailer_license_number_surr_id) ON DELETE CASCADE,
    ...
    CONSTRAINT fct_sales_no_partition_grain_unique UNIQUE (
        game_number_surr_id,
        customer_surr_id,
        employee_surr_id,
        retailer_license_number_surr_id,
...
BEGIN
    ...
    WHILE current_month <= last_month LOOP
        partition_name := 'fct_sales_partitioned_' || to_char(current_month, 'YYYYMM');
        ...
        IF NOT EXISTS (
            ...
            EXECUTE format('
                CREATE TABLE bl_dm.%I PARTITION OF bl_dm.fct_sales_partitioned
...
CREATE OR REPLACE PROCEDURE bl_cl.sp_insert_fct_sales_incremental_partitioned(
    p_start_date DATE,
    p_end_date DATE
)
LANGUAGE plpgsql
...
BEGIN
    WITH ins AS (
        INSERT INTO bl_dm.fct_sales_partitioned (
            game_number_surr_id,
            customer_surr_id,
            employee_surr_id,
            retailer_license_number_surr_id,
        ...
        ON CONFLICT ON CONSTRAINT fct_sales_partitioned_grain_unique DO NOTHING
        RETURNING 1
    )
    SELECT count(*) INTO v_rows_inserted FROM ins;

    CALL bl_cl.p_log_etl(
        p_procedure_name := 'sp_insert_fct_sales_incremental_partitioned',
...
CREATE OR REPLACE PROCEDURE bl_cl.sp_insert_fct_sales_incremental_no_partition(
    p_start_date DATE,
    p_end_date DATE
)
LANGUAGE plpgsql
...
BEGIN
    WITH ins AS (
        INSERT INTO bl_dm.fct_sales_no_partition (
            game_number_surr_id,
            customer_surr_id,
            employee_surr_id,
            retailer_license_number_surr_id,
        ...
        LEFT JOIN bl_dm.dim_date dd ON dd.event_dt = s.event_dt
        WHERE s.event_dt BETWEEN p_start_date AND p_end_date
        ON CONFLICT ON CONSTRAINT fct_sales_no_partition_grain_unique DO NOTHING
        RETURNING 1
    )
    SELECT count(*) INTO v_rows_inserted FROM ins;

    CALL bl_cl.p_log_etl(
        p_procedure_name := 'sp_insert_fct_sales_incremental_no_partition',
        p_rows_affected := v_rows_inserted,
        p_log_message := format('Inserted %s rows into fct_sales for window %s to %s.', v_rows_inserted, p_start_date, p_end_date),
        p_log_level := 'INFO'
    );
...
CREATE OR REPLACE PROCEDURE bl_dm.load_fct_sales_rolling_window_partitioned()
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    WHILE  v_window_start + v_window_size - INTERVAL '1 day' <= v_latest_date
        ...
        BEGIN
            ...
            CALL bl_cl.sp_insert_fct_sales_incremental_partitioned(v_window_start, v_window_end);
            ...
            VALUES ('fct_sales_partitioned', v_window_end, TRUE, CURRENT_TIMESTAMP)
        ...
        EXCEPTION WHEN OTHERS THEN
            ...
            INSERT INTO bl_cl.etl_run_tracker (table_name, last_processed_date, load_success, last_attempted_at)
            VALUES ('fct_sales_partitioned', v_window_end, FALSE, CURRENT_TIMESTAMP)
            ON CONFLICT (table_name) DO UPDATE
...
BEGIN
    ...
    WHILE  v_window_start + v_window_size - INTERVAL '1 day' <= v_latest_date
        ...
        BEGIN
            -- Insert incremental data for this window
            CALL bl_cl.sp_insert_fct_sales_incremental_no_partition(v_window_start, v_window_end);

            -- Update tracker with success
            INSERT INTO bl_cl.etl_run_tracker (table_name, last_processed_date, load_success, last_attempted_at)
            VALUES ('fct_sales_no_partition', v_window_end, TRUE, CURRENT_TIMESTAMP)
            ON CONFLICT (table_name) DO UPDATE
            SET last_processed_date = EXCLUDED.last_processed_date,
                load_success = TRUE,
                last_attempted_at = CURRENT_TIMESTAMP;
        ...
        EXCEPTION WHEN OTHERS THEN
            -- On failure, update tracker with failure status
            INSERT INTO bl_cl.etl_run_tracker (table_name, last_processed_date, load_success, last_attempted_at)
            VALUES ('fct_sales_no_partition', v_window_end, FALSE, CURRENT_TIMESTAMP)
            ON CONFLICT (table_name) DO UPDATE
            SET load_success = FALSE,
                last_attempted_at = CURRENT_TIMESTAMP;
